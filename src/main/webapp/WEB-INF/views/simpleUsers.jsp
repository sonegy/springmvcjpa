...
<c:forEach var="simpleUser" items="${page.content}" varStatus="status">
	<tr>
	    <td>${page.totalElements - (first + status.index)}</td>
		<td><a href="<spring:url value="/simpleUsers/${simpleUser.id}"/>"><c:out value="${simpleUser.name}" /></a></td>
	</tr>
</c:forEach>
</tbody>
</table>

<spring:url value="/simpleUsers" var="next">
   <spring:param name="page.page" value="${page.number + 2}" ></spring:param>
   <spring:param name="page.size" value="${page.size}" ></spring:param>
</spring:url>

<spring:url value="/simpleUsers" var="prev">
...
<br/>
<a href="<spring:url value="/simpleUsers/form"></spring:url>">심플유저등록</a>
</body>
</html>